...
func TestNewOFDReader(t *testing.T) {
	pwd, _ := os.Getwd()
	// 替换成目标文件
	file := filepath.Join(pwd,"samples", "gh11.ofd")

	ofdReader, err := ofd.NewOFDReader(file)
	if err != nil {
		t.Logf("%s", err)
	}
...
func TestBatch(t *testing.T) {
	...
	for _, test := range testcases {
		t.Run(test.name, func(t *testing.T) {
			ofdReader, err := ofd.NewOFDReader(test.path, ofd.WithValidator(&ofd.CommonValidator{}))
			if err != nil {
				t.Logf("%s", err)
			}
			defer ofdReader.Close()
